...
/**
 * volatile关键字，确保一个变量在多线程间可见
 * 假设A, B线程都用到一个变量，Java默认是A线程中保留一份copy, 这样如果B线程修改了该变量，则线程A未必知道。
 ...
 * 
 * Java线程处理的内存模型JMM，深入了解《深入Java虚拟机》
...
public class T9 {
	 ...
	 * CPU中的寄存器(即CPU中的cache)中。为什么这么做呢？
	 * 因为CPU的速度要比内存的速度快，内存的速度比硬盘快。所以要把running中的数据copy一份到内存中处理。
	 * 但是，没有加volatile关键字的变量running，当主线程已经把running改为false，自己启动的线程依然不能停下来。
	 * 因为它读的是CPU中running。主线程改的内存中的running。两个线程读写的变量的存储位置不同。
	 *
